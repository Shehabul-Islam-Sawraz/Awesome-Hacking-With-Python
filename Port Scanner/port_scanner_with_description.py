...
ascii_banner = pyfiglet.figlet_format("PORT SCANNER")   # Creates the text 'PORT SCANNER' as a banner
print(ascii_banner)
...
def get_arguments():
    ...
    target = socket.gethostbyname(options.target)  # Translate a host name to IPv4 address format. If host name is an
...
try:
    for port in range(1, max_port_range+1):
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # AF_INET means IPv4, SOCK_STREAM means TCP
                                                              # So, we are opening a TCP connection on IPv4 with the other end
        socket.setdefaulttimeout(0.1) # Close connection to a port after 1s
